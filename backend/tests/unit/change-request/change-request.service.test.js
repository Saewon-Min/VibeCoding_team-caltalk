...
describe('change-request.service — BE-24 BR-11/BR-12 동시성 단위 테스트', () => {
  const createdUserIds = [];
  const createdTeamIds = [];
  ...
  async function setupTeam({ memberCount = 2 } = {}) {
    const suffix = `${Date.now()}-${Math.floor(Math.random() * 100000)}`;
    const leaderId = await createUser('팀장', `cr-concurrency-leader-${suffix}@test.com`);
    createdUserIds.push(leaderId);
    const teamId = await createTeamWithLeader(`변경요청동시성테스트팀-${suffix}`, leaderId);
    createdTeamIds.push(teamId);

    const memberIds = [];
    for (let i = 0; i < memberCount; i += 1) {
      const memberId = await createUser(`팀원${i}`, `cr-concurrency-member${i}-${suffix}@test.com`);
      createdUserIds.push(memberId);
      await addMembership(teamId, memberId, 'member');
      memberIds.push(memberId);
    }
  ...
  test('동일 schedule의 대기 요청 A/B 중 하나를 승인하면 나머지가 자동 rejected로 전환되며, 어느 쪽을 먼저 승인해도 대칭적으로 동작한다 (BR-11, SC-09 A1)', async () => {
    const { leaderId, teamId } = await setupTeam({ memberCount: 0 });
    ...
    const crA = await changeRequestService.createChangeRequest(teamId, schedule1.id, leaderId, {
      reason: '요청 A',
    });
    const crB = await changeRequestService.createChangeRequest(teamId, schedule1.id, leaderId, {
      reason: '요청 B',
    });

    await changeRequestService.approveChangeRequest('leader', teamId, crA.id, leaderId);

    expect((await getChangeRequestRow(crA.id)).status).toBe('approved');
    expect((await getChangeRequestRow(crB.id)).status).toBe('rejected');

    // A1 대칭 케이스: 나중에 제기된 요청을 먼저 승인해도 동일한 규칙이 대칭적으로 적용된다.
    const schedule2 = await scheduleService.createSchedule('leader', teamId, leaderId, {
      title: '일정2',
      startAt: '2026-07-21T11:00:00.000Z',
      endAt: '2026-07-21T12:00:00.000Z',
      participantUserIds: [leaderId],
    });
    const crC = await changeRequestService.createChangeRequest(teamId, schedule2.id, leaderId, {
      reason: '요청 C',
    });
    const crD = await changeRequestService.createChangeRequest(teamId, schedule2.id, leaderId, {
      reason: '요청 D',
    });

    await changeRequestService.approveChangeRequest('leader', teamId, crD.id, leaderId);

    expect((await getChangeRequestRow(crD.id)).status).toBe('approved');
    expect((await getChangeRequestRow(crC.id)).status).toBe('rejected');
  });

  test('자동 거절된 요청을 뒤늦게 승인 시도하면 409가 발생하고 상태가 유지된다 (BR-11 E1)', async () => {
    const { leaderId, teamId } = await setupTeam({ memberCount: 0 });
    const schedule = await scheduleService.createSchedule('leader', teamId, leaderId, {
      title: '일정',
      startAt: '2026-07-21T09:00:00.000Z',
    ...
    const crA = await changeRequestService.createChangeRequest(teamId, schedule.id, leaderId, {
      reason: '요청 A',
    });
    const crB = await changeRequestService.createChangeRequest(teamId, schedule.id, leaderId, {
      reason: '요청 B',
    });
    ...
    await expect(
      changeRequestService.approveChangeRequest('leader', teamId, crB.id, leaderId),
    ).rejects.toMatchObject({ statusCode: 409 });

    expect((await getChangeRequestRow(crB.id)).status).toBe('rejected');
  });

  test('BR-11 자동 거절로 pending이 아니게 된 요청은 요청자 본인이 취소를 시도해도 409가 발생한다 (BR-12)', async () => {
    const { leaderId, teamId, memberIds } = await setupTeam({ memberCount: 1 });
    const memberId = memberIds[0];
    const schedule = await scheduleService.createSchedule('leader', teamId, leaderId, {
      title: '일정',
      startAt: '2026-07-21T09:00:00.000Z',
      endAt: '2026-07-21T10:00:00.000Z',
      participantUserIds: [leaderId, memberId],
    });
    const crA = await changeRequestService.createChangeRequest(teamId, schedule.id, leaderId, {
      reason: '요청 A',
    });
    const crB = await changeRequestService.createChangeRequest(teamId, schedule.id, memberId, {
      reason: '요청 B',
    });
    ...
    await expect(
      changeRequestService.cancelChangeRequest(teamId, crB.id, memberId),
    ).rejects.toMatchObject({ statusCode: 409 });
  });

  test('요청자 본인이 아닌 사용자의 취소 시도는 거부되고 상태가 pending으로 유지된다 (BR-12)', async () => {
    const { leaderId, teamId, memberIds } = await setupTeam({ memberCount: 2 });
    const [requester, other] = memberIds;
    const schedule = await scheduleService.createSchedule('leader', teamId, leaderId, {
      title: '일정',
    ...
    expect((await getChangeRequestRow(cr.id)).status).toBe('pending');
  });
});
