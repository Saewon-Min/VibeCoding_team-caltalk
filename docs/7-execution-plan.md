...
- [x] 캘린더 일자 선택 시 채팅 패널이 해당 일자로 자동 전환.
- [x] 데스크톱에서 2단 동시 열람 레이아웃.
...
- [x] 자신이 참여자인 일정에 한해 변경 요청 유형 선택 가능(SC-06, BR-10).
- [x] 제안 제목/시간(선택)/사유 필드(ENT-07).
- [x] 제출 성공 시 "대기중" 메시지 즉시 노출(SC-06).
- [x] BR-10 위반 서버 거부 시 오류 노출(SC-06 E1).
- [x] 팀장 계정에는 미노출.
