...
- [ ] 제목/설명/시간/참여자 다중 선택 필드(ENT-05, SC-03).
- [ ] 생성/수정 시 대응 API 호출.
- [ ] 팀원 계정에는 "일정 추가/수정" 버튼 비노출(SC-03 E1).
- [ ] 저장 성공 시 캘린더 즉시 갱신(SC-05).
- 의존성: [ ] FE-13, FE-09, FE-07

**FE-20. 일정 상세보기 + 삭제 확인 모달**
- [ ] 팀원 계정은 읽기 전용, 수정/삭제 버튼 비노출(BR-03).
- [ ] 팀장 계정만 수정/삭제, 삭제는 확인 모달 경유(SC-05).
...
- [ ] SC-01~SC-12 기본 흐름이 문서상 사후조건과 동일하게 재현됨.
- [ ] 모든 예외 흐름(E1, E2 등)이 명시된 상태 코드/메시지대로 동작.
- [ ] BR-01~BR-16 전체 체크리스트 통과(크리티컬 버그 0건).
- [ ] 발견된 버그는 즉시 수정 및 재검증.
- 의존성: [ ] BE-01~BE-24 전체, DB-01~DB-13/15/19 · 근거: SC-01~SC-12, PRD 5.1절

**BE-26. 배포 설정 및 최종 점검**
- [ ] `.env` 설정으로 서버 정상 기동 및 DB 연결.
- [ ] `.env.example`이 최신 환경변수와 일치.
- [ ] 캐시/오토스케일링/CI-CD 등 범위 외 인프라 미추가 확인.
- [ ] 재시작 후 기존 데이터 유지 확인.
- 의존성: [ ] BE-25
